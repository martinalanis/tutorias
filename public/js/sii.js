        ...
        _createClass(Mark, [{
            ...
            value: function createDiacriticsRegExp(str) {
                var sens = this.opt.caseSensitive ? "" : "i",
                    dct = this.opt.caseSensitive ? ["aÃ Ã¡Ã¢Ã£Ã¤Ã¥ÄÄ…Äƒ", "AÃ€ÃÃ‚ÃƒÃ„Ã…Ä€Ä„Ä‚", "cÃ§Ä‡Ä", "CÃ‡Ä†ÄŒ", "dÄ‘Ä", "DÄÄŽ", "eÃ¨Ã©ÃªÃ«Ä›Ä“Ä™", "EÃˆÃ‰ÃŠÃ‹ÄšÄ’Ä˜", "iÃ¬Ã­Ã®Ã¯Ä«", "IÃŒÃÃŽÃÄª", "lÅ‚", "LÅ", "nÃ±ÅˆÅ„", "NÃ‘Å‡Åƒ", "oÃ²Ã³Ã´ÃµÃ¶Ã¸Å", "OÃ’Ã“Ã”Ã•Ã–Ã˜ÅŒ", "rÅ™", "RÅ˜", "sÅ¡Å›È™ÅŸ", "SÅ ÅšÈ˜Åž", "tÅ¥È›Å£", "TÅ¤ÈšÅ¢", "uÃ¹ÃºÃ»Ã¼Å¯Å«", "UÃ™ÃšÃ›ÃœÅ®Åª", "yÃ¿Ã½", "YÅ¸Ã", "zÅ¾Å¼Åº", "ZÅ½Å»Å¹"] : ["aÃ Ã¡Ã¢Ã£Ã¤Ã¥ÄÄ…ÄƒAÃ€ÃÃ‚ÃƒÃ„Ã…Ä€Ä„Ä‚", "cÃ§Ä‡ÄCÃ‡Ä†ÄŒ", "dÄ‘ÄDÄÄŽ", "eÃ¨Ã©ÃªÃ«Ä›Ä“Ä™EÃˆÃ‰ÃŠÃ‹ÄšÄ’Ä˜", "iÃ¬Ã­Ã®Ã¯Ä«IÃŒÃÃŽÃÄª", "lÅ‚LÅ", "nÃ±ÅˆÅ„NÃ‘Å‡Åƒ", "oÃ²Ã³Ã´ÃµÃ¶Ã¸ÅOÃ’Ã“Ã”Ã•Ã–Ã˜ÅŒ", "rÅ™RÅ˜", "sÅ¡Å›È™ÅŸSÅ ÅšÈ˜Åž", "tÅ¥È›Å£TÅ¤ÈšÅ¢", "uÃ¹ÃºÃ»Ã¼Å¯Å«UÃ™ÃšÃ›ÃœÅ®Åª", "yÃ¿Ã½YÅ¸Ã", "zÅ¾Å¼ÅºZÅ½Å»Å¹"];
                var handled = [];
...
// Autosend form
$(document).on('change', '.autosend', function(e){
	e.preventDefault()
...
$(document).ready( function(e) {
	$('.modal-autoshow').each( function(i, el) {
		$(el).modal('show')
	})
})
...
// Collapse onClick at checkbox
$(document).on('click', '.matrix', function(e) {
...
$(document).on('keyup', '.form-search', function(e) {
	let term = $(this).val()
	setTimeout(function(){
		search_context.unmark().show()
		if(term) {
			search_context.mark(term, {
				done: function() {
					search_context.not(":has(mark)").hide()
				}
			})
		}
	}, 1000)
...
// Checkboxes like radio button behavior
$(".checkbox-like-radio").change(function() {
    $(".checkbox-like-radio").prop('checked', false);
    $(this).prop('checked', true);
})

// Calculate average on scores capture
function calculateAvg(ncontrol) {
	let average = 0
	let counter = 0
	let units = true
	$('.unit[data-ncontrol="'+ncontrol+'"]').each(function(idx, el) {
	...
	$('.final[data-ncontrol="'+ncontrol+'"]').val(average)
}

$(document).on('keyup', '.unit', function(e) {
	let ncontrol = $(this).data('ncontrol')
	setTimeout(function() {
		calculateAvg(ncontrol)
	}, 100)
})

$(document).ready(function(e) {
	$('.final').each(function(idx, el) {
		let ncontrol = $(el).data('ncontrol')
		if(!$(el).val()) {
			calculateAvg(ncontrol)	
		}
	})
...
// Get cities from states for school search
let state_id = null
$(document).on('change', '.state-school', function(e) {
	e.preventDefault()
	state_id = $(this).val()
	let action = $(this).data('action') + '/' + state_id	
	$.ajax(action).done(function(res) {
		if(res.error == true) {
			return new PNotify({ title: 'Error', text: res.message, type: 'error', icon: false })
		} else {
			$('.dynamic-city').remove()
			$('.dynamic-school').remove()
			$.each(res.cities, function (i, city) {
				$('.city-school').append($('<option>', {
					value: city.municipio.trim(),
					text: city.municipio.trim().capitalize(),
					class: 'dynamic-city'
				}))
			})
			$('.city-school-block').removeClass('hidden')
		}
	}).fail(function(err) {
		return new PNotify({ title: 'Error', text: 'No se pudo cargar los municipios de la escuela de procedencia, intÃ©ntalo de nuevo', type: 'error', icon: false })
	})
})

// Get schools from city
let city_id = null
$(document).on('change', '.city-school', function(e) {
	e.preventDefault()
	city_id = $(this).val()
	let action = $(this).data('action') + '/' + state_id + '/' + city_id
	$.ajax(action).done(function(res) {
		if(res.error == true) {
			return new PNotify({ title: 'Error', text: res.message, type: 'error', icon: false })
		} else {
			$('.dynamic-school').remove()
			$.each(res.schools, function (i, school) {
				$('.school').append($('<option>', {
					value: school.clave_preparatoria.trim(),
					text: school.nombre_preparatoria.trim().capitalize(),
					class: 'dynamic-school'
				}))
			})
			$('.school-block').removeClass('hidden')
		}
	}).fail(function(err) {
		return new PNotify({ title: 'Error', text: 'No se pudo cargar los municipios de la escuela de procedencia, intÃ©ntalo de nuevo', type: 'error', icon: false })
	})
...
// Function to check and uncheck all checkbox
function checkUncheckAll(selecteds) {
	let inputs = document.getElementsByTagName('input')
	if (selecteds.checked) {
		for (let i = 0; i < inputs.length; i++) {
			if (inputs[i].type == 'checkbox' && inputs[i].name.includes('aceptados')) {
...
// Function to add rows on table
$(document).on('click', '.btn-saad', function(e) {
	if($(this).data('content') == 'add') {
		if($(this).attr('id') == 'addA' || $(this).attr('id') == 'addB') {
			// a or b
			let x
			if($(this).attr('id') == 'addA')
				x = 'a'
			else 
				x = 'b'
			// Validate data
			if($('#'+x+'1').val() == null || $('#'+x+'2').val() == null || $('#'+x+'3').val() == null || $('#'+x+'4').val() == null || $('#'+x+'5').val() == null)
				return new PNotify({ title: 'Error', text: 'Ningun dato debe ir vacio', type: 'error', icon: false })
			// Get table
			let table = document.getElementById('table' + x.toUpperCase())
			let row = table.insertRow(1);
			// Insert data
			row.insertCell(0).innerHTML = $('#'+x+'1').val()+'<input type="hidden" name="asignaturas_'+x+'[]" value="'+$('#'+x+'1').val()+'" form="solicitud" />'
			row.insertCell(1).innerHTML = $('#'+x+'2').val()+'<input type="hidden" name="temas_'+x+'[]" value="'+$('#'+x+'2').val()+'" form="solicitud" />'
			row.insertCell(2).innerHTML = $('#'+x+'3').val()+'<input type="hidden" name="docentes_'+x+'[]" value="'+$('#'+x+'3').val()+'" form="solicitud" />'
			row.insertCell(3).innerHTML = $('#'+x+'4').val()+'<input type="hidden" name="periodos_'+x+'[]" value="'+$('#'+x+'4').val()+'" form="solicitud" />'
			row.insertCell(4).innerHTML = $('#'+x+'5').val()+'<input type="hidden" name="instructores_'+x+'[]" value="'+$('#'+x+'5').val()+'" form="solicitud" />'
			// Action button
			let actionCell = row.insertCell(5)
			actionCell.className = 'text-center'
			actionCell.innerHTML = '<a data-content="del" class="btn btn-raised btn-danger btn-mini btn-saad">Eliminar</a>'
			// Clear inputs
			$('#'+x+'1').val('')
			$('#'+x+'2').val('')
			$('#'+x+'3').val('')
			...
			// aa or bb
			let x
			if($(this).attr('id') == 'addAA')
				x = 'aa'
			else 
				x = 'bb'
			// Validate data
			if($('#'+x+'1').val() == null || $('#'+x+'2').val() == null || $('#'+x+'3').val() == null)
				return new PNotify({ title: 'Error', text: 'Ningun dato debe ir vacio', type: 'error', icon: false })
			// Get table
			let table = document.getElementById('table' + x.toUpperCase())
			let row = table.insertRow(1);
			// Insert data
			row.insertCell(0).innerHTML = $('#'+x+'1').val()+'<input type="hidden" name="actividades_'+x+'[]" value="'+$('#'+x+'1').val()+'" form="solicitud" />'
			row.insertCell(1).innerHTML = $('#'+x+'2').val()+'<input type="hidden" name="carreras_'+x+'[]" value="'+$('#'+x+'2').val()+'" form="solicitud" />'
			row.insertCell(2).innerHTML = $('#'+x+'3').val()+'<input type="hidden" name="fechas_'+x+'[]" value="'+$('#'+x+'3').val()+'" form="solicitud" />'
			// Action button
...
$(document).ready(function() {
	...
	$('#addr'+i).append("<td class='text-center'> <div class='form-group is-empty'>  <input name='ncontrol[]' type='text'  class='form-control'/> </div> </td> <td class='text-center'> <div class='radio'>  <label>    <input type='radio' name='op["+i+"]'  value='1' tabindex='-1'>  <span class='circle'> </span> <span class='check'> </span> </label> </div> </td> <td class='text-center'> <div class = 'radio'> <label>  <input  name='op["+i+"]' type='radio' value='2' tabindex='-1'> <span class='circle'> </span> <span class='check'> </span> </label> </div> </td>");
